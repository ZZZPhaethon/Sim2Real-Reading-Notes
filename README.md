...
### 0.3 从七篇论文总结出的研究范式：表示先行

阅读这些工作后可以发现，其中相当一部分采用了**表示驱动（representation-driven）**的研究路线：研究者首先定义一种连接视觉输入、生成模型和仿真资产的中间表示，再围绕该表示构造数据、设计模型、实现解码器并建立评测。

```text
研究问题
   -> 定义中间表示：模型究竟需要预测什么？
   -> 构造数据与监督：如何获得这种表示的训练标签？
   -> 设计生成或重建模型：如何从图像预测该表示？
   -> 解码 / 编译：如何转换为 Mesh、PBR、URDF、MJCF 或 USD？
   -> 任务评测：结果是否具有视觉质量、几何质量和物理可执行性？
```

中间表示并不只是数据格式。它同时决定了：

- **学习目标**：网络预测体素、SLAT、部件 token、关节参数，还是文本化物理描述；
- **数据需求**：数据集需要提供几何、材质、部件分割、关节轴、运动范围或动力学属性中的哪些标注；
- **模型结构**：适合使用 VLM 自回归、扩散/flow matching、稀疏卷积，还是检索与程序化组装；
- **输出能力**：结果能否被稳定转换成可编辑、可渲染或可仿真的资产；
- **评价标准**：应使用图像与几何指标，还是进一步检查 URDF 加载、关节运动、碰撞和机器人交互。

七篇工作的侧重点并不完全相同，可以分为三类：

| 类型 | 代表工作 | 主要研究逻辑 |
|---|---|---|
| **表示驱动型** | TRELLIS.2、PhysX-Anything、PhysX-Omni、PhysForge | 提出或扩展 O-Voxel、体素文本序列、统一物理几何描述、Physical Blueprint/KineVoxel 等表示，并据此组织数据、训练模型和设计评测。 |
| **模型结构驱动型** | PAct | 复用 TRELLIS/SLAT 等已有表示，重点创新部件内注意力、多阶段部件生成，以及从去噪特征回归关节参数。 |
| **系统集成型** | GS-Playground、LiteReality | 主要组合已有表示、资产与工具，解决 3DGS 和刚体运动绑定、CAD 检索、场景组装、材质迁移及仿真执行问题。 |

因此，不能把所有论文都概括成“提出新表示，再构建数据集并训练模型”。更准确的结论是：

> **该方向的核心研究对象是从视觉观测到可执行三维资产的接口。部分工作通过发明新的中间表示改造这一接口，部分工作改进表示上的生成模型，另一些工作则负责将已有表示接入真实的重建、渲染和仿真系统。**

这一观察也给出了开展新工作的基本检查表：

1. **Representation**：模型应预测哪些视觉、结构和物理变量？
2. **Dataset**：这些变量的监督来自真实标注、仿真生成、资产库转换还是自动伪标注？
3. **Model**：模型需要解决感知、补全、生成、物理推理中的哪一部分？
4. **Compiler/Decoder**：如何无歧义地转换为仿真器可读资产？
5. **Evaluation**：如何证明结果不仅“看起来合理”，而且能够稳定加载、运动和交互？

从这组文献出发，一个值得继续研究的缺口是：现有表示已经能够编码较高质量的几何、材质和部分关节信息，但**表示到仿真器之间仍缺少统一、可靠的验证与修正闭环**。因此，后续工作既可以继续设计更完整的 simulation-ready 表示，也可以研究生成资产的自动检查、物理修正和任务级评测。
...
```text
打基础：
  0.1~0.4 基础概念 → TRELLIS(D) 三维生成表示 → 3DGS(C) 三维重建表示
